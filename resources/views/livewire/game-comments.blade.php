<div class="mt-6 comments-section">
    ...
    @guest
        <p class="text-gray-700">Devi <a href="{{ route('login') }}" class="text-blue-500">accedere</a> per lasciare un
            commento.</p>
    @else
        <div class="mb-4">
    ...
    <div class="comments-list">
        @foreach ($comments as $comment)
            <div class="p-4 mb-4">
                <p class="font-semibold">{{ $comment['author_name'] ?? 'Anonimo' }}</p>
                <p class="mt-2">{{ $comment['content'] }}</p>
                <small
                    class="text-gray-500">{{ \Carbon\Carbon::parse($comment['created_at'])->diffForHumans() }}</small>
